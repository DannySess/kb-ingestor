#!/usr/bin/env python3
"""
kb-ingestor v3: Multi-KB routing, file update detection, config-driven.
Two-level dedup: KB index + file store index.
"""
...
_file_index  = {}       # filename -> file_id (Open WebUI file store)
...
def build_file_store_index():
    """Build index of ALL files in Open WebUI file store: filename -> file_id"""
    index = {}
    page = 1
    while True:
        try:
            r = requests.get(
                f"{OPEN_WEBUI_URL}/api/v1/files/?page={page}&limit=100",
            ...
            log.warning(f"Could not fetch file store page {page}: {e}")
            break
    log.info(f"  File store index: {len(index)} file(s)")
    return index


def build_kb_index(kb_id: str) -> dict:
    """Build index for a specific KB: filename -> file_id"""
    try:
        r = requests.get(
...
def ingest_file(path: Path) -> bool:
    ...
    # File changed — delete old versions
    if path.name in kb_idx:
        ...
        if path.name in _file_index:
            del _file_index[path.name]

    # Check if already uploaded but not in KB
    if path.name in _file_index:
        file_id = _file_index[path.name]
        log.info(f"  📎 Already uploaded, adding to KB: {path.name}")
    else:
...
def startup_scan():
    global _file_index
    _file_index = build_file_store_index()
    ...
    if failed_files:
        ...
        time.sleep(30)
        # Refresh file store index before retry
        _file_index = build_file_store_index()
...
def main():
    ...
    log.info("=" * 60)
    log.info("kb-ingestor v3 starting")
